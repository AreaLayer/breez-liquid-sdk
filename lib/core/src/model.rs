...
impl ChainSwap {
    ...
    pub(crate) fn get_claim_swap_script(&self) -> Result<SwapScriptV2, PaymentError> {
        ...
        let swap_script = match self.payment_type {
            PaymentType::Receive => SwapScriptV2::Liquid(LBtcSwapScriptV2::chain_from_swap_resp(
                Side::To,
                chain_swap_details,
                our_pubkey.into(),
            )?),
            PaymentType::Send => SwapScriptV2::Bitcoin(BtcSwapScriptV2::chain_from_swap_resp(
                Side::To,
                chain_swap_details,
                our_pubkey.into(),
            )?),
    ...
    pub(crate) fn get_lockup_swap_script(&self) -> Result<SwapScriptV2, PaymentError> {
        ...
        let swap_script = match self.payment_type {
            PaymentType::Receive => SwapScriptV2::Bitcoin(BtcSwapScriptV2::chain_from_swap_resp(
                Side::From,
                chain_swap_details,
                our_pubkey.into(),
            )?),
            PaymentType::Send => SwapScriptV2::Liquid(LBtcSwapScriptV2::chain_from_swap_resp(
                Side::From,
                chain_swap_details,
                our_pubkey.into(),
            )?),
