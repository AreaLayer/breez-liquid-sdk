...
use boltz_client::{BtcSwapScriptV2, LBtcSwapScriptV2, ToHex};
...
impl LiquidSdk {
    ...
    pub async fn send_onchain_payment(
        ...
        let create_response = self.swapper.create_chain_swap(CreateChainRequest {
            ...
            server_lock_amount: Some(receiver_amount_sat as u32), // TODO update our model
            pair_hash: Some(lbtc_pair.hash),
            referral_id: None,
        })?;
        ...
        let payer_amount_sat = req.fees_sat + receiver_amount_sat;
        let claim_fees_sat = lbtc_pair.fees.claim_estimate();
        ...
        let lockup_script = LBtcSwapScriptV2::chain_from_swap_resp(
            Side::From,
            create_response.lockup_details.clone(),
            refund_public_key,
        )?;
        let lockup_res = lockup_script.validate_address(
            Chain::Liquid,
            create_response.lockup_details.lockup_address.clone(),
        );
        debug!("Lockup res: {:?}", lockup_res);

        let claim_script = BtcSwapScriptV2::chain_from_swap_resp(
            Side::To,
            create_response.claim_details.clone(),
            claim_public_key,
        )?;
        let claim_res = claim_script.validate_address(
            Chain::Bitcoin,
            create_response.claim_details.lockup_address.clone(),
        );
        debug!("Claim res: {:?}", claim_res);
